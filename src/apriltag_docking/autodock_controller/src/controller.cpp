...
void autodock_controller::searching_state_fun(){
    ...
    if (tag_callback_counter<lost_tag_max){
        ...
        set_action_state("");
        RCLCPP_INFO(get_logger(),"last_tag_y:%f,tag_y:%f" , last_tag_y,tag_y);
        double effective_tag_y=in_view?tag_y:last_tag_y;
        neuron_turn(default_turn*sign(effective_tag_y));
    }
}
...
void autodock_controller::final_approach_state_fun(){
    ...
    if (in_view and fabs(pose_set.distance-finish_distance)>0.2){
        set_docking_state("approach");
        return;
    }
...
void autodock_controller::fid2pos(){
    ...
    if (r > 1.5){theta_bounds = approach_angle;}
     else{theta_bounds = r/30.0;}
    //else{theta_bounds = approach_angle;}
