      ...
      "apriltag_detections", rclcpp::QoS(10)))
{
  ...
  const auto ids = declare_parameter<std::vector<int64_t>>("tag_ids", {0});
  const auto frames = declare_parameter<std::vector<std::string>>("tag_frames", {"dock_frame"});
  ...
  const auto sizes = declare_parameter<std::vector<double>>("tag_sizes", {0.18});
  if (!sizes.empty()) {
  ...
  const sensor_msgs::msg::CameraInfo::ConstSharedPtr & msg_ci)
{
  ...
  //image_u8_t im = {.width = img_uint8.cols,.height = img_uint8.rows,
  //  .stride = img_uint8.cols,.buf = img_uint8.data};
  ...
  // std::cout<<"image_u8_t: "<<im.width<<" "<<im.height<<" "<<im.stride<<" "<<im.buf<<std::endl;
  std::mutex mutex;
  // detect tags
  mutex.lock();
...
//upupup
    // Draw detected tag corners on the image
    for (int j = 0; j < 4; j++) {
      cv::line(
        img_uint8,
  ...
  cv::imshow("AprilTag Detections", img_uint8);
